...
**Revision:** Updated to integrate interface-engine / policy-broker workflows, XCA-I / Carequality imaging gateway patterns, governance overlay concepts (CSS / PEP / PDP / Trust Registry), reference deployment patterns, and advanced-site CDC / resiliency / observability guidance (2026-03-12)

---

**Table of Contents**

- **Part I: Design & Architecture** (Sections 1-19) — Problem statement, transport category taxonomy, governance overlay, site classification, transport mechanisms by category, security, operations, advanced-site enhancements
- **Part II: Implementation Plan** (Tasks 0-17) — TDD code, module map, dependency graph, rationale framework
- **Governance Overlay** — CSS / PEP / PDP / Trust Registry mapped to the current CAT-A through CAT-E implementation model
- **Appendix E** — Reference deployment patterns carried forward from the prior blueprint
- **Advanced-Site Enhancements** — CDC / event-driven updates, resumable transfer, and richer observability without changing the baseline nightly model
...
| **EHR-Mediated Exchange** | `CAT-B` | The EHR's built-in clinical image exchange infrastructure is repurposed for research retrieval. The EHR handles all PACS communication; RADIANT receives DICOM through a standard exchange protocol. | Epic Care Everywhere Image Exchange, Epic Radiant IHE Profiles (RAD-69/RAD-55), other EHR-native clinical exchange workflows proven to return full diagnostic studies |
| **Middleware-Brokered Retrieval** | `CAT-C` | The site's existing API gateway, reverse proxy, integration engine, or cross-community imaging gateway acts as a **stateful policy broker**. RADIANT presents a signed StudyRelease / delegated release artifact; the broker validates identity and signature, persists an allow-list cache, optionally translates local identifiers, enforces revocation and replay prevention, and only then forwards to the PACS read-only API, DICOM gateway, or cross-community imaging gateway. | F5 BIG-IP, NGINX Plus, Kong, Apigee, MuleSoft Anypoint, Azure APIM, Rhapsody, Cloverleaf, Corepoint, InterSystems Health Connect / IRIS for Health / HealthShare, Hyland Acuo Cross Community Gateway Manager, Mirth Connect, Iguana, Qvera, Epic Bridges/Interconnect, AWS API Gateway, open eHealth IPF-based custom gateways |
...
START: RADIANT EHR Connector is operational, Bulk FHIR exports running
  ...
  |     (F5, Kong, Apigee, MuleSoft, Rhapsody, HealthShare, Acuo Cross Community Gateway, etc.)
  |
...
This maximizes use of infrastructure the site already operates and trusts — Epic Bridges / Interconnect, Rhapsody, Cloverleaf, Corepoint, HealthShare, Mirth, Iguana, Qvera, F5, Apigee, Kong, MuleSoft, Azure APIM, and similar platforms. In governance-overlay terms (Section 2.6), these platforms act as the local **PEP**, and when they call an internal rule route or policy API they also host the optional **PDP**.
...
### 2.6 Governance Overlay — CSS / PEP / PDP / Trust Registry

The CAT-A through CAT-E taxonomy remains the **normative implementation model** for this document. This governance overlay reintroduces the prior blueprint's control-plane vocabulary so that privacy, security, and institutional architecture reviewers can reason about the same design using familiar concepts without replacing the newer category model.

| Governance concept | Role | Concrete implementation in this plan |
|--------------------|------|--------------------------------------|
| **CSS (Cohort Snapshot Service)** | Signed statement of which studies may be retrieved | **Study Manifest** + consent proof + retrieval ledger; optional manifest-derived snapshot summary distributed to gateways |
| **StudyRelease** | Retrieval-scoped derivative of the snapshot | Signed artifact naming patient/study scope, TTL, allowed operations, manifest hash, and correlation metadata |
| **PEP (Policy Enforcement Point)** | Allow/deny/proxy retrieval requests | CAT-C policy broker, site API gateway, interface engine, or XCA-I Initiating Imaging Gateway operating in RADIANT mode |
| **PDP (Policy Decision Point)** | Optional real-time site veto or supplemental policy decision | Rhapsody / Cloverleaf / Corepoint / Health Connect rule route, custom webhook, or site policy API |
| **TR (Trust Registry)** | Signed system of record for site metadata and trust anchors | Site configuration + secrets + probe results today; optional externalized Trust Registry service as the network scales |

In practical terms, the current architecture maps to the overlay as follows:

1. **Bulk FHIR discovery** provides the input set from which CSS builds the study-granular authorization state.
2. **Study Manifest** is the operational cohort snapshot at study granularity.
3. **StudyRelease** is the retrieval-scoped authorization artifact derived from that snapshot.
4. **CAT-C gateway / policy broker** is the default PEP implementation because it validates the StudyRelease before PACS access occurs.
5. **Optional site PDP** gives the institution a real-time veto hook for withdrawal, local restriction, or temporary operational hold.
6. **Trust Registry** stores the site-specific identities, endpoints, keys, rate limits, maintenance windows, HomeCommunityIDs, AE titles, and gateway metadata needed to operate safely at scale.

This overlay does **not** force a new central service on day one. A minimal deployment can treat `site_config` + Secrets Manager + Task 0 probe results as the seed form of the Trust Registry. As the program scales, those same fields can be externalized into a dedicated signed registry without changing the transport categories.

```mermaid
flowchart LR
    BFHIR[Bulk FHIR export
registry-scoped] --> CSS[CSS
manifest-derived snapshot]
    CSS --> MANIFEST[Study Manifest]
    MANIFEST --> RELEASE[StudyRelease
signed delegated release artifact]
    TR[Trust Registry
endpoints keys limits policies] --> RELEASE
    TR --> PEP[PEP / policy broker]
    RELEASE --> PEP
    PEP --> PDP{Optional PDP
site veto / local policy}
    PDP -->|ALLOW| RETRIEVE[Validated transport
CAT-A / CAT-C / CAT-D / CAT-E]
    PDP -->|DENY| AUDIT[Audit / deny event]
    PEP -->|No PDP or cached allow| RETRIEVE
    RETRIEVE --> LEDGER[Retrieval ledger
consent proof + audit]
    AUDIT --> LEDGER

    style CSS fill:#264653,color:#fff
    style RELEASE fill:#2a9d8f,color:#fff
    style PEP fill:#e9c46a,color:#000
    style TR fill:#457b9d,color:#fff
    style LEDGER fill:#6d597a,color:#fff
    style AUDIT fill:#e63946,color:#fff
```

### 2.7 Governance Overlay Adoption Notes

- **Do not replace the CAT model** with CSS / PEP / PDP / TR terminology; use the overlay to explain governance and control-plane responsibilities.
- **Default PEP locus** for research retrieval is the site-owned broker in CAT-C, because that is where StudyRelease validation is explicit and auditable.
- **PDP is optional**. Sites that do not want a live veto service can rely on short-lived StudyRelease TTLs plus periodic consent re-verification.
- **Trust Registry contents** should be limited to operational metadata: no PHI, no DICOM pixels, and no large manifests. It stores trust anchors and routing metadata, not payloads.
...
| Does the site operate an API gateway, integration engine, or cross-community imaging gateway (XCA-I)? | CAT-C viability | `CAT-C` Middleware-Brokered Retrieval |
...
| XCA-I / HIE imaging probe | Healthcheck to configured Initiating Imaging Gateway / Responding Imaging Gateway endpoints plus optional sandbox ITI-38/RAD-75 test where available | `CAT-C` — whether a cross-community imaging gateway can serve as the institutionally approved broker for authorized retrieval |
...
class SiteCapabilityReport:
    ...
    cat_c_xcai: ProbeResult = ProbeResult(status="not_tested")
    cat_c_xcai_product: str | None = None  # "intersystems", "acuo", "ipf", etc.
    cat_c_home_community_id: str | None = None
    ...
    recommended_transport: str  # "cat_a_dicomweb", "cat_a_fhir_binary", "cat_c_gateway", "cat_c_xcai_gateway", etc.
...
RADIANT issues a **StudyRelease** authorization (the default and recommended release artifact in this architecture, embodied as a signed study ticket or equivalent control-plane message) naming the exact study to be retrieved. The site's broker validates the authorization, persists a short-lived allow-list entry, applies local policy, and only then invokes the PACS-facing read-only API or DICOM gateway using site-managed credentials.
...
#### Variant 1: RADIANT-Issued StudyRelease / Delegated Release Artifact (Recommended Baseline)

This is the default and recommended CAT-C pattern. The **RADIANT Connector itself** issues the release artifact because it already possesses the approved Bulk FHIR-derived manifest and consent chain. The release artifact is therefore a **delegated, manifest-derived authorization object** naming the exact study that may be retrieved.

The site broker treats the StudyRelease as the authoritative bridging artifact between the authorization plane and the retrieval plane. Epic or another EHR does **not** need to mint a second authorization object unless local governance explicitly requires it.

- **Trust chain:** site-approved ingress auth (typically mTLS) + JWKS-signed StudyRelease / study ticket issued by RADIANT
- **Broker validates:** caller identity, signature, expiration, study UID / accession match, one-time token ID, consent-proof hash if provided
- **Replay prevention:** broker tracks `ticket_id` in a short-lived cache aligned to ticket lifetime
- **Why this is sufficient:** the artifact is derived directly from the EHR-approved, registry-scoped manifest; the broker still remains the only system trusted to access PACS
- **Best fit:** API gateways, reverse proxies, healthcare integration engines, XCA-I Initiating Imaging Gateways

#### Variant 2: Epic-Endorsed / Epic-Originated Release Artifact (Optional — Requires Epic TS Confirmation)

For sites that want Epic to remain the **visible institutional authorization authority** even for CAT-C retrieval. In this variant, RADIANT requests a release artifact from Epic or an Epic-owned service; the site broker only accepts retrieval if that Epic-endorsed or Epic-originated release is present and valid.

This can be attractive institutionally, but it is **not required** for the core CAT-C trust model and is **not established** by the provided Epic documentation. Treat this as an optional governance / optics variant pending Epic Technical Services confirmation.

#### Variant 3: Token Exchange (RFC 8693 / RFC 7662) (Proposed — Requires Site Broker Support)

For brokers that support standards-based token exchange: RADIANT presents an Epic / RADIANT token or StudyRelease; the broker introspects or validates it and mints a one-time broker-scoped retrieval token. This is the most standards-shaped pattern, but support varies widely by platform.

### 9.8 CAT-C Subpattern: Cross-Community Imaging Gateway (XCA-I / XDS-I.b / Carequality Imaging)

Some sites or regional HIE environments already operate a **cross-community imaging gateway** rather than a simple PACS proxy. In that case, the gateway should be treated as a **CAT-C policy broker** rather than as CAT-B clinical image exchange. The key distinction is that the gateway is an institutionally operated middleware / HIE component, not merely an EHR workflow feature.

In Carequality-style or XDS-I.b-based exchange, the local gateway first obtains an **imaging manifest / DICOM KOS** through XCA query flows and then retrieves the referenced images through **XCA-I**. This is highly compatible with the RADIANT architecture because the KOS manifest plays a role similar to the RADIANT StudyManifest: it constrains which images may be requested before bytes are released.

#### 9.8.1 Verified Middleware Support for XCA-I

| Product / Middleware | Support Status | What is verified | Recommended RADIANT role | Source basis |
|----------------------|----------------|------------------|--------------------------|--------------|
| **InterSystems Health Connect / IRIS for Health / HealthShare** | **Verified native support** | Explicit XCA-I documentation for Initiating Imaging Gateway, Responding Imaging Gateway, and registering XDS-I.b sources | Full CAT-C cross-community imaging broker | InterSystems XCA-I scenario + IHE registry settings docs |
| **Hyland Acuo Cross Community Gateway Manager** | **Verified native support** | Supports Initiating / Responding Gateway functions for XCA, XCA-I, XCPD, XCDR, XCF; explicit config for XCA-I responding gateways, imaging document sources, and XCA-I test app | Full CAT-C cross-community imaging broker | Hyland Acuo Cross Community Gateway Manager technical specs and config docs |
| **open eHealth IPF** | **Verified framework support** | Components for ITI-38 (XCA query), RAD-69, and RAD-75 (XCA-I) | Custom-built CAT-C gateway or engineering accelerator, not turnkey middleware | IPF IHE component docs |
| **Qvera Interface Engine** | Public evidence of Carequality / network connectivity, but **no explicit public XCA-I actor support located** | Can likely serve as control-plane broker or custom integration host | Treat as CAT-C policy broker or custom implementation pending vendor confirmation | Qvera Carequality connectivity + platform docs |
| **Rhapsody / Cloverleaf / Corepoint / Mirth / Iguana / generic API gateways** | **No explicit public XCA-I actor support verified in this review** | Strong candidates for control-plane brokering, SOAP orchestration, and local policy enforcement | Use as CAT-C policy broker; treat native XCA-I as custom / vendor-confirmed only | Public product docs reviewed did not explicitly document XCA-I actors |
| **Epic Interconnect / Bridges** | **Not verified as native XCA-I actor support in reviewed Epic docs** | Reviewed docs show DICOM gateway and exchange workflows, but not explicit XCA-I actor roles | Treat as DICOM gateway / policy broker candidate, not a verified XCA-I stack | Epic Interconnect / Care Everywhere docs reviewed for this plan |

#### 9.8.2 How to Leverage the RADIANT Authorized List with XCA-I

The **RADIANT Connector authorized list remains the source of truth**. XCA-I does not replace the authorization plane; it becomes a retrieval-plane implementation of CAT-C. The best-fit pattern is:

1. **RADIANT Bulk FHIR** produces the approved StudyManifest with patient / study scope
2. RADIANT issues a **StudyRelease** naming the patient, StudyInstanceUID, accession, and allowed operations
3. The local **Initiating Imaging Gateway (IIG)** / policy broker validates that StudyRelease and persists a short-lived allow-list
4. The broker performs **patient discovery / XCA query** only for the authorized patient and requests imaging-manifest / KOS entries
5. The broker filters returned KOS / document entries against the authorized StudyUID / accession from StudyRelease
6. The broker performs **RAD-75** only for matching, authorized entries
7. The broker returns images to the landing zone and emits `StudyReady` / `RetrievalFailed` back to RADIANT

This pattern prevents XCA-I from becoming a broad patient query surface. The broker is only allowed to use XCA-I in service of the exact studies already authorized by the RADIANT Connector.

#### 9.8.3 RADIANT-to-XCA-I Workflow

```mermaid
sequenceDiagram
    participant R as RADIANT Connector
    participant IIG as Site IIG / Policy Broker
    participant XCPD as Patient Discovery / XCA
    participant RIG as Responding Imaging Gateway
    participant IDS as XDS-I.b Imaging Document Source

    R->>IIG: StudyRelease (signed, patient + StudyUID + accession)
    IIG->>IIG: Validate signature, expiry, replay, local policy
    IIG->>XCPD: XCPD / XCA query for authorized patient
    XCPD-->>IIG: DICOM KOS / imaging manifest entries
    IIG->>IIG: Filter KOS entries to authorized StudyUID / accession only
    IIG->>RIG: RAD-75 Retrieve Imaging Document Set
    RIG->>IDS: Retrieve referenced DICOM objects
    IDS-->>RIG: DICOM objects
    RIG-->>IIG: DICOM objects
    IIG-->>R: StudyReady / RetrievalFailed + correlation metadata
```

#### 9.8.4 Practical Mapping Rules

For CAT-C / XCA-I sites, the broker should map RADIANT fields to XCA-I retrieval inputs as follows:

| RADIANT field | XCA-I / KOS usage | Notes |
|---------------|-------------------|-------|
| `patient_fhir_id` | Query scope / patient discovery correlation | Usually translated to local / community patient identifiers by the broker |
| `study_uid` | Primary filter against KOS references | Strongest image-level authorization key |
| `accession_number` | Secondary filter / reconciliation key | Useful when KOS metadata is incomplete or local mapping is accession-centric |
| `manifest_id` / `ticket_id` | Audit / correlation only | Should not be sent as an imaging key; keep for audit and callback correlation |
| `allowed_operations` | Broker policy guard | For XCA-I, normally authorize query + KOS retrieval + RAD-75 retrieve only |

#### 9.8.5 FHIR + XCA-I Hybrid Pattern

FHIR remains the preferred **authorization and discovery control plane** even when XCA-I is the retrieval mechanism. In this hybrid pattern:

- **FHIR / Bulk FHIR / ImagingStudy** identify which studies are authorized
- **StudyRelease** carries that authorization to the site broker
- **XCA / XCA-I** retrieve the KOS manifest and then the DICOM objects

FHIR does **not** replace RAD-75 or DICOM object retrieval in this model. It governs *what may be retrieved*; XCA-I governs *how the images are fetched across communities*.

### 9.9 Healthcare API Gateways and Interface Engines
...
### 9.10 Retrieval Ledger Integration
...
Step 6: DICOM Retrieval via validated transport
           ...
           +-- CAT-C (direct broker): policy broker -> DICOM gateway / PACS read-only API
           +-- CAT-C (XCA-I): policy broker / Initiating Imaging Gateway -> XCA query/KOS -> RAD-75
...
release artifact to site broker"]
    REL --> TC{"CAT-C subpattern?"}
    TC -->|"Direct broker"| TCD["Policy broker validates,
queues, and invokes PACS
read-only API"]
    TC -->|"XCA-I gateway"| TCX["Initiating Imaging Gateway validates,
queries KOS, and performs RAD-75"]
...
| CAT-C (Middleware / Policy Broker) | RADIANT-issued StudyRelease + broker allow-list cache + local policy / identifier translation; for XCA-I, KOS filtering + RAD-75 only for authorized studies | Broker endpoint only (mTLS / site-approved ingress) |
...
### 12.5 Cross-Community Imaging Controls (CAT-C / XCA-I)

When CAT-C is implemented using a cross-community imaging gateway, the broker should enforce the following additional controls:

- Treat the **RADIANT StudyRelease** as the only acceptable trigger for XCA-I retrieval
- Limit XCPD / XCA query scope to the exact patient authorized by the StudyRelease
- Filter returned **DICOM KOS / manifest entries** to the authorized `study_uid` and/or `accession_number` before any image retrieval occurs
- Permit **RAD-75** only for KOS entries that survived that authorization filter
- Persist `homeCommunityId`, `repositoryUniqueId`, `documentUniqueId`, and broker correlation IDs for audit and retry
- Deny broad patient-browse use of the IIG when operating in RADIANT mode

This keeps XCA-I aligned with the same principle used elsewhere in the architecture: **bytes may only be retrieved after a manifest-derived authorization artifact is validated.**

### 12.6 Governance Overlay Mapping

The table below makes the governance overlay operational by showing where each concept lives in the current implementation model.

| Overlay concept | Minimum viable implementation now | Program-scale implementation later |
|-----------------|-----------------------------------|------------------------------------|
| **CSS** | Study Manifest builder + retrieval ledger + consent proof sidecars | Dedicated service that signs manifest-derived snapshot summaries for gateways |
| **PEP** | CAT-C policy broker, XCA-I IIG in RADIANT mode, or validated CAT-A proxy | Standardized broker package / site deployment pattern catalogue |
| **PDP** | Optional webhook / rule route in existing middleware | Shared policy contract with standardized deny reasons and SLAs |
| **Trust Registry** | `site_config` + secrets + Task 0 probe outputs | Externalized signed registry of site identities, endpoints, cert refs, limits, and maintenance windows |

This keeps the retrieval architecture aligned with the older governance model without requiring a separate greenfield control plane at day one.
...
Phase 1: Assessment
  ...
  [ ] If site / HIE operates XCA-I: capture product, homeCommunityId, ITI-38 / RAD-75 endpoints, and governing exchange framework (Carequality, regional HIE, private affinity domain)
...
Phase 3: RADIANT Configuration (parallel with Phase 2)
  ...
  [ ] If CAT-C / XCA-I: configure IIG / RIG endpoints, homeCommunityId allow-list, KOS filtering rules, and RAD-75 timeout / retry policy
...
Phase 4: Validation
  ...
  [ ] If CAT-C / XCA-I: verify patient discovery / XCA query, KOS retrieval and filtering to authorized studies only, RAD-75 retrieval, and callback correlation metadata
...
governance:
  snapshot_mode: "manifest_derived"  # CSS overlay uses the study manifest as the study-granular snapshot
  pep_mode: "site_policy_broker"     # local PEP implementation for CAT-C
  pdp_enabled: true
  pdp_endpoint: "https://rhapsody.seattlechildrens.org/radiant/v1/authorize"
  trust_registry_record_id: "seattle-childrens-v1"
  maintenance_window_local: "Sun 01:00-03:00"
...
This section defines the **baseline** monitoring model required for go-live. Section 18 adds optional advanced-site enhancements such as ATNA export, OpenTelemetry tracing, synthetic monitoring, and resumable-transfer instrumentation.
...
## 18. Future / Advanced-Site Enhancements

The baseline operating model for this plan remains **nightly manifest-driven retrieval using a probe-validated transport category**. The capabilities in this section are optional enhancements for mature sites and should not block initial onboarding. They are intended to reduce latency, improve recovery from long-running transfers, and deepen institutional observability while preserving the same StudyManifest / StudyRelease authorization boundary.

### 18.1 Change Data Capture (CDC) and Event-Driven Consent / Study Updates

Advanced sites may supplement the nightly Bulk FHIR refresh with control-plane events that update authorization or retrieval timing more quickly than the next export window. Examples include:

- **Consent / eligibility changes** delivered through HL7 v2 ADT-style change events, registry membership feeds, or site policy webhooks
- **Study availability / completion events** delivered through HL7 ORU/ORM, DICOM MPPS, archive webhooks, or platform callbacks
- **Membership-change fanout** through interface engines, queues, Kafka, Kinesis, SNS/SQS, or equivalent infrastructure the site already operates

These feeds should never bypass the manifest boundary. Instead, they should trigger one of four controlled actions: `StudyRelease`, `StudyRevoke`, `StudyReady`, or `RetrievalFailed`.

```mermaid
sequenceDiagram
    participant BF as Nightly Bulk FHIR
    participant CSS as Manifest / CSS Overlay
    participant EI as Event Ingest (optional)
    participant PB as Site Policy Broker
    participant PACS as PACS / DICOM Gateway

    BF->>CSS: Nightly cohort refresh
    CSS->>PB: StudyRelease (baseline)

    par Optional event-driven updates
      EI->>CSS: Consent change / registry delta
      CSS->>PB: StudyRevoke or updated StudyRelease
    and
      EI->>PB: Study availability / completion event
      PB->>PB: Queue early retrieval candidate
    end

    PB->>PACS: Retrieve only if current StudyRelease valid
```

### 18.2 Resumable Transfer and Integrity Protection

For large cross-sectional studies, mature sites may want stronger recovery semantics than a simple restart-on-failure model. Recommended advanced capabilities include:

- **Per-series or per-instance checkpoint state** so interrupted studies can resume without discarding completed work
- **Transport-aware resume strategies** such as HTTP Range or part-level restart when the upstream transport supports it
- **Immutable checksum manifests** (SHA-256 preferred) for each DICOM object and for the completed study inventory
- **Session persistence** so long-running retrievals survive broker restarts or temporary network interruptions
- **Idempotent object keys** based on Study/Series/SOP UIDs so retries never create duplicate lineage

These features are most useful for CAT-C, CAT-D, and XCA-I deployments where large studies may cross multiple retries or handoffs.

### 18.3 Richer Observability and Institutional Audit

Baseline logs and daily reports are sufficient for initial deployment, but advanced sites may also want:

- **Correlation IDs** carried from Bulk FHIR export -> manifest -> StudyRelease -> broker -> retrieval -> validation -> S3 landing
- **Prometheus / Grafana or equivalent** for gateway throughput, retry depth, queue age, and transport-health metrics
- **IHE ATNA or SIEM export** for sites that require centralized security-event retention
- **OpenTelemetry traces** for end-to-end request timing across orchestrator, broker, and platform callbacks
- **Synthetic health checks** that exercise low-risk probe paths on a schedule and alert on drift before nightly retrieval fails

These capabilities should remain optional and site-specific; they are an enhancement to operational maturity, not a prerequisite for go-live.

### 18.4 Adoption Guardrails

To keep advanced features from destabilizing onboarding, use the following guardrails:

1. **Nightly manifest-driven retrieval remains the authoritative baseline** until an event-driven path is validated.
2. **Event feeds may accelerate** release, revoke, or ready signals, but they may not authorize retrieval outside the manifest / StudyRelease chain.
3. **Resumable transfer logic must preserve original DICOM objects** and only create derivative metadata or resumable checkpoints alongside them.
4. **Advanced observability must remain transport-neutral** so CAT-A through CAT-E sites can adopt richer telemetry incrementally.
...
| **Survey site middleware gateways / interface engines** — which sites already have F5, Kong, Apigee, MuleSoft, Rhapsody, Cloverleaf, Corepoint, HealthShare, Mirth, Iguana, Qvera, Epic Bridges / Interconnect, Acuo Cross Community Gateway, or custom XCA-I stacks? | Pending | Determines CAT-C policy-broker viability and preferred ingress pattern per site |
...
| **XCA-I-capable middleware inventory** — which sites/HIE partners already run InterSystems Health Connect / HealthShare, Hyland Acuo, or another Initiating Imaging Gateway? | Pending | Determines whether CAT-C / XCA-I can be used without net-new software |
| **StudyRelease-to-KOS mapping** — what is the cleanest site implementation for filtering KOS / document entries to authorized `study_uid` / accession before RAD-75? | Pending | Core design question for CAT-C / XCA-I deployments |
| **Trust Registry externalization** — when should `site_config` + secrets + probe results become a dedicated signed service? | Future | Governance overlay scales cleanly either way; externalization improves rotation and audit at network scale |
| **Site PDP patterns** — which sites want real-time veto hooks and which middleware products are easiest to standardize? | Future | Likely candidates: Rhapsody, Cloverleaf, Corepoint, Health Connect, custom policy APIs |
| **CDC feed contract mapping** — which sites can surface consent/status/study-availability events without net-new software? | Future | HL7 ADT/ORU, DICOM MPPS, archive webhooks, queue fanout |
| **Resumable transfer capability matrix** — which upstream transports support part-level resume or range-style restart semantics? | Future | Important for large CAT-C / CAT-D / XCA-I studies |
| **Advanced observability export** — which sites require ATNA, SIEM export, or OpenTelemetry traces? | Future | Optional advanced-site enhancement, not onboarding gate |
...
| Event-driven retrieval triggered by validated site feeds (HL7 ORU/ORM, MPPS, archive webhook) | Future | Covered by Section 18; enable only after baseline nightly model is stable |
| **Study ticket revocation** — mechanism for revoking issued tickets before expiration | Future | Section 18 outlines event-driven revoke patterns; productize only where site PDP / queue support exists |
...
### 19.1 Historical Backfill Architecture (Conceptual Only — Not Operational)
...
## Appendix D: XCA-I / Carequality Imaging Gateway Integration

### D.1 Why XCA-I Belongs in CAT-C

Cross-Community Access for Imaging (XCA-I) is best modeled in this architecture as a **CAT-C subpattern** when it is implemented by a site-operated gateway, HIE broker, or institutional cross-community exchange stack. The local Initiating Imaging Gateway acts as the trusted retrieval broker. RADIANT still supplies the authorization source of truth via Bulk FHIR + StudyManifest + StudyRelease.

### D.2 Normative / Product Observations Used in This Plan

- Carequality imaging exchange requires Query Initiators / Responders to provide **XCA-I gateway actors**, and the inter-community image retrieval transaction is **RAD-75**
- InterSystems explicitly documents **Initiating Imaging Gateway**, **Responding Imaging Gateway**, and **Registering XDS-I.b Sources**
- Hyland Acuo explicitly documents XCA-I configuration for initiating and responding gateways and testing of XCA-I transactions
- open eHealth IPF provides engineering-level components for **ITI-38**, **RAD-69**, and **RAD-75**, which is useful for custom gateway builds

### D.3 Recommended RADIANT Pattern for XCA-I

```
RADIANT Bulk FHIR / ImagingStudy
    -> StudyManifest
    -> StudyRelease (signed)
    -> Site IIG / policy broker
    -> XCPD / XCA query for KOS
    -> KOS filter to authorized StudyUID / accession only
    -> RAD-75 retrieve images
    -> StudyReady / RetrievalFailed callback
```

### D.4 Support Matrix (Reviewed for This Plan)

| Middleware / Product | Support classification | Notes |
|----------------------|------------------------|-------|
| InterSystems Health Connect / IRIS for Health / HealthShare | Verified native XCA-I support | Best documented turnkey path for an institutional IIG / RIG |
| Hyland Acuo Cross Community Gateway Manager | Verified native XCA-I support | Strong turnkey path for cross-community imaging brokers |
| open eHealth IPF | Verified framework support | Good custom-build substrate, not turnkey enterprise middleware by itself |
| Qvera | Not publicly verified as native XCA-I in this review | Good candidate for policy broker or vendor-confirmed custom XCA-I |
| Rhapsody / Cloverleaf / Corepoint / Mirth / Iguana / generic API gateways | Not publicly verified as native XCA-I in this review | Best treated as CAT-C policy brokers unless vendor confirms XCA-I actors |
| Epic Interconnect / Bridges | Not publicly verified as native XCA-I in reviewed Epic docs | Use for DICOM gateway / broker patterns, not assumed XCA-I |

### D.5 Design Rule

When XCA-I is present, **never let it become a broad patient-browse surface for RADIANT.** Always require a StudyRelease first, and always filter KOS / document entries to the exact authorized study scope before RAD-75 retrieval.

---

## Appendix E: Reference Deployment Patterns (Carried Forward from Prior Blueprint)

The CAT-A through CAT-E taxonomy in Part I remains the **normative implementation model**. This appendix preserves the most useful deployment patterns from the earlier blueprint and maps them to the current categories so that sites can recognize familiar operating models without changing the current architecture.

### E.1 Pattern Summary Table

| Prior-pattern shorthand | Current mapping | Best fit | Enforcement locus |
|-------------------------|----------------|----------|-------------------|
| **Pattern A — Central PEP with manifest-derived snapshot checks** | Usually validated CAT-A or site-exposed CAT-C | Site already exposes a read-only imaging API and is comfortable with central proxying | Central PEP / RADIANT-managed broker |
| **Pattern B — Central PEP + site PDP** | CAT-C overlay | Site wants a real-time veto service while keeping a shared ingress path | Central PEP + site PDP webhook / rule route |
| **Pattern C — Site-hosted policy broker / edge connector** | Canonical CAT-C | Best institutional fit when the site wants local control at the PACS boundary | Site-owned gateway / interface engine / edge service |
| **Pattern D — Cross-community imaging gateway (IIG / RIG)** | CAT-C / XCA-I subpattern | Site or HIE already operates XCA-I actors | Initiating Imaging Gateway + optional local PDP |
| **Pattern E — PACS-adjacent platform / VNA front-end** | CAT-D | Site already operates Ambra / VNA / enterprise imaging platform | Platform API / routing layer |

### E.2 Pattern A — Central PEP with Manifest-Derived Snapshot Checks

Use this when a site already exposes a validated read-only imaging API and the institution accepts central proxying. RADIANT distributes a manifest-derived snapshot summary or StudyRelease to the central PEP, which allows retrieval only for enumerated StudyUIDs. This is closest to the older "central PEP" model, but in the current architecture it should only be used where the site's governance team accepts that enforcement locus.

### E.3 Pattern B — Central PEP + Site PDP

Use this when a site is comfortable with central ingress but wants a local veto hook for withdrawal, temporary hold, or local restriction logic. The site PDP does not move DICOM bytes; it returns an allow / deny decision with optional policy reason codes. This maps naturally to Rhapsody / Cloverleaf / Corepoint / Health Connect routes or lightweight policy webhooks.

### E.4 Pattern C — Site-Hosted Policy Broker / Edge Connector

This is the preferred institutional pattern for CAT-C. The broker receives `StudyRelease`, validates signature and TTL, persists allow-list state, performs local identifier translation and optional PDP calls, then invokes the site-managed PACS-facing API or DICOM gateway. This is the clearest trust boundary for security review because retrieval permissions are enforced inside the site's own integration stack.

### E.5 Pattern D — Cross-Community Imaging Gateway (XCA-I)

Use this when the site or its affiliated HIE already operates an Initiating Imaging Gateway / Responding Imaging Gateway. In this pattern, the IIG is treated as the PEP. RADIANT still issues `StudyRelease`, but the IIG performs XCPD / XCA query, filters returned KOS entries to the authorized study scope, and issues RAD-75 only for those entries.

### E.6 Pattern E — PACS-Adjacent Platform / VNA Front-End

Use this when an institutional imaging platform already provides PACS connectivity, validation, and routing. RADIANT sends the manifest or StudyRelease to the platform API; the platform becomes the enforcement and retrieval layer. This keeps direct PACS details out of the RADIANT runtime while preserving institutional ownership of image movement.

```mermaid
flowchart LR
    PA[Pattern A
Central PEP] --> MA[Validated CAT-A or site-exposed CAT-C]
    PB[Pattern B
Central PEP + Site PDP] --> MB[CAT-C overlay]
    PC[Pattern C
Site policy broker / edge] --> MC[Canonical CAT-C]
    PD[Pattern D
IIG / RIG] --> MD[CAT-C / XCA-I]
    PE[Pattern E
Platform / VNA front-end] --> ME[CAT-D]

    style PA fill:#264653,color:#fff
    style PB fill:#2a9d8f,color:#fff
    style PC fill:#e9c46a,color:#000
    style PD fill:#457b9d,color:#fff
    style PE fill:#6d597a,color:#fff
```
...
lib/
  imaging/                          # NEW — all DICOM retrieval modules
    ...
    transport_xcai_gateway.py       # Task 8b: XCA-I / Carequality imaging gateway client
    ...
    xca_i_mapping.py                # Task 8b: StudyRelease -> KOS / RAD-75 mapping + filtering
    cohort_snapshot_overlay.py      # Governance overlay: manifest-derived CSS summaries / snapshot exports
    trust_registry.py               # Governance overlay: signed site metadata / endpoints / key inventory
    policy_decision_client.py       # Optional PDP webhook / rules-engine adapter
    event_ingest.py                 # Advanced-site CDC / webhook / HL7 event intake
    transfer_resumption.py          # Advanced-site checkpoint / checksum / resume state
    observability.py                # Correlation IDs, ATNA export, OpenTelemetry helpers
...
tests/
  ...
  test_transport_xcai_gateway.py    # Task 8b tests
  ...
  test_xca_i_mapping.py             # Task 8b tests
...
- Cross-community imaging gateways: InterSystems Health Connect / HealthShare, Hyland Acuo Cross Community Gateway Manager, custom IPF-based IIG/RIG deployments
- Hybrid deployments: interface engine for control plane + Ambra/VNA / DICOM gateway for retrieval plane

### Task 8b: XCA-I / Carequality Imaging Gateway Support

For CAT-C sites that already operate an **Initiating Imaging Gateway (IIG)**, the broker client must support an alternate retrieval path:

1. Accept RADIANT-issued `StudyRelease`
2. Translate it into the minimal query context required for XCPD / XCA imaging-manifest discovery
3. Retrieve KOS / imaging-manifest metadata
4. Filter returned entries to the authorized `study_uid` / `accession_number` before retrieval
5. Invoke **RAD-75** only for matching entries
6. Persist `homeCommunityId`, `repositoryUniqueId`, `documentUniqueId`, and transfer-syntax decisions for retry / audit

Minimum Task 8b tests should verify:

- StudyRelease-to-query mapping
- KOS filtering to authorized studies only
- RAD-75 request construction
- handling of zero-match / multi-match KOS conditions
- callback correlation with `StudyReady` / `RetrievalFailed`
...
Variant 1 — RADIANT-Issued StudyRelease / Delegated Release Artifact:
    RADIANT signs a StudyRelease with the same JWKS/key trust chain used for
    the approved backend app. The site gateway validates that delegated release
    artifact and forwards to PACS or another retrieval broker. Gateway examples:
    F5, Apigee, Kong, MuleSoft, Azure APIM, Rhapsody, InterSystems HealthShare,
    vendor DICOMweb gateways, and XCA-I Initiating Imaging Gateways.

Variant 2 — Epic-Endorsed / Epic-Originated Release Authorization:
    Optional governance / optics pattern. The site gateway requires an Epic-
    originated or Epic-endorsed release artifact before using the PACS
    credential. This gives the site stronger optics: Epic itself authorized
    the release. This is not the baseline requirement.
...
The nightly batch coordinator. Loads config, builds manifest, deduplicates against ledger, retrieves via transport adapter, validates, writes consent proofs, generates daily report. Nightly operation remains the authoritative baseline; advanced sites may additionally feed event-driven updates into the same state model (Section 18).
...
class RetrievalOrchestrator:
    ...
    def process_control_plane_event(self, event: ControlPlaneMessage) -> None:
        """
        Optional advanced-site path:
        - Accept StudyRevoke / StudyReady / RetrievalFailed / consent delta events
        - Update broker / ledger state without waiting for the next nightly cycle
        - Never authorize retrieval outside the current StudyManifest / StudyRelease chain
        """
```
...
## Decision 15: Governance Overlay (CSS / PEP / PDP / Trust Registry) as an Explanatory Layer
- **Decision:** Reintroduce CSS / PEP / PDP / Trust Registry as a governance overlay without replacing the CAT-A through CAT-E implementation model.
- **Rationale:** The newer category model is stronger for execution, while the older CSS/PEP/PDP/TR vocabulary is stronger for privacy, compliance, and institutional architecture review. Using the overlay gives governance audiences a familiar control-plane model while preserving the current transport taxonomy and CAT-C study-ticket design.
- **Operational effect:** StudyManifest acts as the study-granular snapshot, StudyRelease is the retrieval-scoped derivative, CAT-C brokers act as the default PEP, and site config / secrets / probe outputs act as the initial Trust Registry.

## Decision 16: Advanced-Site Enhancements Are Optional Overlays, Not Baseline Dependencies
- **Decision:** CDC/event-driven updates, resumable transfer, and richer observability are future / advanced-site overlays and must not become prerequisites for onboarding.
- **Rationale:** The nightly manifest-driven model is easier to validate, easier for sites to operate, and already satisfies the consent / authorization boundary. Advanced features improve timeliness and resilience for mature sites but should layer onto the same StudyManifest / StudyRelease state model rather than replace it.
- **Guardrail:** No event-driven or resumable feature may authorize retrieval outside the manifest-derived boundary.
...
- Trust Registry / certificate / endpoint metadata maintenance
- Optional PDP and StudyRelease revocation procedures
- Advanced-site CDC event handling and replay troubleshooting
- Resumable transfer recovery and checksum reconciliation
- Optional ATNA / SIEM / OpenTelemetry export guidance
...
- Governance overlay checklist (CSS / PEP / PDP / Trust Registry fields)
- Advanced-site readiness checklist (CDC, resumability, richer telemetry)
- Go-live procedure
...
**Note on CAT-C:** Task 8 now covers both the RADIANT-side broker client and the control-plane contracts / state model that a site broker must implement. Task 8b adds the XCA-I / Carequality imaging-gateway subpattern for sites that already operate an Initiating Imaging Gateway. Governance-overlay helpers (`cohort_snapshot_overlay`, `trust_registry`, optional PDP adapters) and advanced-site modules (`event_ingest`, `transfer_resumption`, `observability`) are optional extensions layered onto the same baseline state model.
